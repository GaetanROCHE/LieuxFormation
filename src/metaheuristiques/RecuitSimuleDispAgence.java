...
public class RecuitSimuleDispAgence extends Heuristique{
    ...
    public Solution voisinage(Solution solution1)
    {
        ...
        do{
            ...
            CentreFormation CeRandom = this.getCentres().get(nombreAleatoire2);//un centre al�atoire
            ...
            if(map.get(AgRandom) == CeRandom)
            {
                do{
                    ...
                    CeRandom = this.getCentres().get(nombreAleatoire2);//un centre al�atoire
    ...
    public Solution findSolution(ArrayList<Integer> Identifiants){//Recherche d'une solution pour les centres propos�s
        ...
        for(Agence ag : this.getAgences()){//pour chaque agence
            ...
            for (int id : Identifiants)    //on parcours chaque centre
            {
                ...
                //V�rification de la validit� du centre
                int i = 0;
                boolean check = true;
                    for (int j=0; j<map.size(); j++) {
                        if (map.get(j) == ce) {
                ...
                if(distance<min && check)// si c'est la distance min et qu'il y a moins de 60 agences attrib�es � ce centre, on conserve la distance et le centre en question
                {
                    min = distance;
                    meilleurCentre = ce;
                }
            }
            map.put(ag, meilleurCentre);// � la fin on ajoute � la map, la ville avec le centre le plus proche
        }
